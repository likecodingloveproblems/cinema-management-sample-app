...
class Movie(models.Model):
    ...
    # also if it's not correct we must create a unique field for customers.
    title = models.CharField(_("name"), max_length=127, unique=True)
    poster = models.ImageField(_("poster"),
                               upload_to='movie',
                               height_field=None,
                               width_field=None,
                               max_length=None
    ...
    class Meta:
        ...
        constraints = [
            # this will enforce new poster for each movie
...
class Seat(models.Model):
    ...
    room = models.ForeignKey(
        ...
        verbose_name=_("room"),
        on_delete=models.CASCADE)
    # seat_id is not primary key because it is used by operational team and
    # in a scenario that operation team want to change seat_id for it's
    # internal management reasons it can make issues in tickets,
    # because customer don't take attention to id, take attention to position
    # of seat
    seat_id = models.IntegerField(_("seat_id"))
    # we assume rooms are rectangular, for other shapes we must have other
    # options
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(
                fields=['room', 'row', 'column'],
                name='unique_room_row_column'),
            models.UniqueConstraint(
                fields=['room', 'seat_id'],
                name='unique_room_seat_id'), ]
...
class Ticket(models.Model):
    ...
    # user can play different roles: buyyer and owner, if we have gifting
    # process we must change ticket model.
    buyyer = models.ForeignKey(
        User, verbose_name=_("buyyer"),
        on_delete=models.CASCADE)
